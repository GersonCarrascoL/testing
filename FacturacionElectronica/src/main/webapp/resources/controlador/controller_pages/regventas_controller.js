app.controller("regventas_controller",function($scope,$http){

    var hoy= new Date(); 
    var mes=hoy.getMonth()+1;
    var anio=hoy.getFullYear();
    var cod_dep=$('#idDependencia').text();
    ...
    $http.get("rest/ComprobantePagoController/listarEstablecimientosXFacultad").success(function(data){
    	if(data.length>0){
    		$scope.facultades = data; 
        	$scope.facultad = $scope.facultades[0].udId;
    	}    	
	});
    ...
    function llenarMoneda(){
    	for(var i=1;i<=2;i++){
    		switch(i){
    				case 1: var coin={'id' :38,'name' : 'Soles'};
							$scope.coins.push(coin);
							break;
    				case 2: var coin={'id' :40,'name' : 'Dolares'};
							$scope.coins.push(coin);
							break;
    		}
    	}
    ...
    $scope.actualizarAccion=function(){
    	...
    	$scope.accionC="rest/RegistroVenta/generarReporteRegistroVentasXMes/"+usuario+"/1/"+$scope.facultad+"/"+$scope.year+"/"+$scope.month;
    	$scope.accionD="rest/RegistroVenta/generarReporteRegistroVentasXMes/"+usuario+"/2/"+$scope.facultad+"/"+$scope.year+"/"+$scope.month;
    };
    
    $scope.actualizarAccionxI=function(){
    	$scope.fec_ini=($scope.fechaComienzo).split('/').join('-');
    	$scope.fec_fin=($scope.fechaFinal).split('/').join('-');
    	$scope.accionA="rest/RegistroVenta/generarReporteRegistroVentasXIntervalo/"+usuario+"/1/"+$scope.facultad+"/"+$scope.fec_ini+"/"+$scope.fec_fin;
    	$scope.accionB="rest/RegistroVenta/generarReporteRegistroVentasXIntervalo/"+usuario+"/2/"+$scope.facultad+"/"+$scope.fec_ini+"/"+$scope.fec_fin;
    };
