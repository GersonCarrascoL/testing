app.controller('mantenimiento_notaCreditoBoleta_controller', ['$scope', '$filter', 'ngTableParams', 'comprobantesPagoService', 'notaService','facturacion', function($scope, $filter, ngTableParams, comprobantesPagoService, notaService,facturacion) {
    ...
    $scope.cambio=function(c,i){
    	...
    	var documento={'num_ruc': '20148092282','tip_doc' : '07','num_doc'	: 'B'+c.codEst+"-"+c.serie};
		facturacion.postDirectiva($scope.urlConsultaFacturador,documento).then(
	   	          function(resp) { 
	   	        	  if(resp.status=="0001" || resp.status=="0002"){
	   	        		  	documento={'num_ruc': '20148092282','tip_doc' : '07','num_doc'	: 'B'+c.codEst+"-"+c.serie,'ind_situ':resp.status};	   	        		  
	   	        		  	...
	   	        		  	banderaEnvio=true;
				   	        mensajeCambio("#confirmacion","Enviar","¿Seguro que desea enviar la Nota de Crédito B"+c.codEst+"-"+c.serie+"?");
				   	     	$scope.cTemp=c;
				   	     	$scope.indice=i;
			   	        	$scope.modalSpinnerShow=false;
	   	        	  }
	...
	$scope.verPDF=function(establecimiento,serie) {   	
        window.open("rest/ReporteController/generarPDF/"+establecimiento+"/"+serie+"/"+5+"/"+$scope.doblePagina+"/", "", "width=800,height=600");
    };
    
    $scope.consultar=function(){
    	$scope.getComprobantesDePago( {'tipo': 5, 'fechaInicial': $scope.fechaInicial, 'fechaFinal': $scope.fechaFinal});
    };
    ...
    $scope.volverAEnviar=function(c, i){
    	$scope.modalSpinnerShow = true;
    	$scope.documento={'num_ruc': '20148092282','tip_doc' : '07','num_doc'	: 'B'+c.codEst+"-"+c.serie};
