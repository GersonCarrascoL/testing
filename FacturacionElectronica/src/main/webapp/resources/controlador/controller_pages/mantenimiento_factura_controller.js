app.controller("mantenimiento_factura_controller",function($scope, $http,facturacion){
	var sistema;
    $scope.estados=[{id:"",title:"Todos"},
                    {id:'0',title:"Enviado Aceptado SUNAT"},
	...
	$scope.download=function(a,b){
    	$scope.url="http://quipucamayoc.unmsm.edu.pe/bajada/cxc_CP-20148092282-01-F"+a+"-"+b+".pdf";
    	$scope.abrirModal("#descargarPDF");
    };
    $scope.cambio=function(c,i){
    	$scope.cargando.valor=true;
    	var banderaEnvio=false;
    	var documento={'num_ruc': '20148092282','tip_doc' : '01','num_doc'	: 'F'+c.codEst+"-"+c.serie};
		facturacion.postDirectiva(sistemaConsulta,documento).then(
	   	          function(resp) { 
	   	        	  if(resp.status=="0001" || resp.status=="0002"){
	   	        		  	documento={'num_ruc': '20148092282','tip_doc' : '01','num_doc'	: 'F'+c.codEst+"-"+c.serie,'ind_situ':resp.status};
	   	        		  	...
	   	        		  	banderaEnvio=true;
				   	        $scope.mensajeCambio("#confirmacion","Enviar","¿Seguro que desea enviar la Factura "+c.codEst+"-"+c.serie+"?");
				   	     	$scope.cTemp=c;
				   	     	$scope.indice=i;
    ...
    $scope.cambiarColor=function(c,i){
    	$scope.cargando.valor=true;
    	var factura={'comprobante': 2,'establecimiento' : c.codEst,'serie'	: c.serie};
    	var documento={'num_ruc': '20148092282','tip_doc' : '01','num_doc'	: 'F'+c.codEst+"-"+c.serie};
    	if(c.id=='00'|| c.id=='01'|| c.id=='12'){
	    		facturacion.post('rest/ComprobantePagoController/reenviar',factura).then(
		   	          function(resp) { 
